...
window.__iobrokerDmWidgets__ = { ...DmWidgets, StateContext };
window.__iobrokerShared__ = {
    react: React,
    'react-dom': ReactDOM,
...
console.log('[MF] Host React version:', React.version, '| React identity:', React.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED ? 'OK' : 'MISSING INTERNALS');
 ...
 */
export async function loadPluginComponent(url, adapterName, componentName, admin) {
    ...
    if (!(setPromise instanceof Promise)) {
        try {
            ...
            setPromise = // load translations
                loadRemote(`${uniqueName}/translations`)
                    ...
                    .then(() => loadRemote(`${uniqueName}/Components`));
    ...
    try {
        const pluginReact = module?.__esModule ? undefined : module?.React;
    ...
    if (!Component) {
        ...
        throw new Error(`Plugin component "${componentName}" not found from adapter "${adapterName}" (url: ${url}). Available: ${available}`);
    }
    componentCache.set(cacheKey, Component);
    return Component;
}
/** Check if a plugin component is already loaded (cached) */
export function isPluginLoaded(adapterName, componentName) {
 ...
 */
export function getPluginConfigSchema(adapterName, componentName) {
    const Component = componentCache.get(`${adapterName}/${componentName}`);
    if (!Component) {
        return null;
    }
    const getSchema = Component.getConfigSchema;
