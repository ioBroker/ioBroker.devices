...
export class WidgetUniversal extends WidgetGeneric {
    ...
    componentDidUpdate(prev) {
        ...
        if (prev.settings.stateId !== this.props.settings.stateId ||
        ...
        if (prev.settings.quickChart !== this.props.settings.quickChart ||
    ...
    getQuickBufferKeepMs() {
        const windowSec = Math.max(QUICK_CHART_MIN_WINDOW_SEC, Number(this.props.settings.quickChartInterval) || QUICK_CHART_DEFAULT_WINDOW_SEC);
        return Math.ceil(windowSec * 1000 * 1.2);
    }
    captureQuickSample(value) {
        const val = value ?? this.state.value;
        if (val == null || !(Number.isFinite(val) || val === true || val === false)) {
            return;
        }
        const now = Date.now();
        const sample = { ts: now, val: typeof val === 'boolean' ? (val ? 1 : 0) : val };
    ...
    renderQuickChart() {
        ...
        const line = isBoolean
            ? pts
                .map((p, j) => j === 0 ? `M${p.x.toFixed(1)},${p.y.toFixed(1)}` : `H${p.x.toFixed(1)}V${p.y.toFixed(1)}`)
                .join('')
        ...
        return (React.createElement(Box, { sx: {
            ...
            React.createElement("svg", { viewBox: `0 0 ${W} ${H}`, preserveAspectRatio: "none", style: { width: '100%', height: '100%', display: 'block' } },
                React.createElement("path", { d: `${line} L${lastX},${H} L${firstX},${H} Z`, fill: color, opacity: 0.15 }),
                React.createElement("path", { d: line, fill: "none", stroke: color, strokeWidth: "1.5", opacity: 0.5 }))));
    }
    /** Get a StateObject from cache or fetch it once */
    async getCachedObject(id) {
        const cached = this.objectCache.get(id);
        if (cached) {
            return cached;
        }
        const obj = await this.props.stateContext?.getObject(id);
        if (obj) {
    ...
    subscribe() {
        ...
        if (this.props.settings.stateId) {
            ...
            this.primaryHandler = (_id, state) => {
                ...
                if (this.props.settings.quickChart &&
                    value != null &&
    ...
    static renderIcon(iconDef, active, iconSize = 18) {
        if (!active || !iconDef.icon) {
            return null;
        }
        return (React.createElement(Icon, { src: iconDef.icon, style: { width: iconSize, height: iconSize, color: iconDef.color || undefined } }));
    }
    /**
     ...
     * too aggressively.
     */
    static calcFontSize(text, kind, layout) {
    ...
    renderTile(isWide, isWideTall) {
        ...
        let iconSrc = this.state.actionActive && this.props.settings.widgetIconActive
        ...
        // without overflowing.
        const layoutKind = isWideTall ? 'wideTall' : isWide ? 'wide' : 'compact';
        ...
        return (React.createElement(Box, { sx: theme => ({
            ...
            React.createElement(Box, { onClick: clickable ? this.handleTileClick : undefined, onPointerDown: clickable ? this.handlePointerDown : undefined, onPointerUp: clickable ? this.clearLongPressTimer : undefined, onPointerLeave: clickable ? this.clearLongPressTimer : undefined, onPointerCancel: clickable ? this.clearLongPressTimer : undefined, sx: (theme) => ({
                ...
                indicators,
                secondaryValue != null || this.props.settings.secondaryName ? (React.createElement(Box, { sx: {
                        position: 'absolute',
                        top: 6,
                        right: 8,
                        textAlign: 'right',
                    } },
                    secondaryValue != null ? (React.createElement(Typography, { variant: "caption", sx: {
                        ...
                        secondaryUnit ? (React.createElement(Typography, { component: "span", sx: { fontSize: '0.8em', ml: 0.25, opacity: 0.7 } }, secondaryUnit)) : null)) : null,
                    this.props.settings.secondaryName ? (React.createElement(Typography, { variant: "caption", sx: {
                            fontSize: 10,
                            lineHeight: 1,
                            color: 'text.disabled',
                            display: 'block',
                        } }, this.props.settings.secondaryName)) : null)) : null,
                activeIcons.length > 0 ? (React.createElement(Box, { sx: {
                        position: 'absolute',
                        top: 8,
                        left: 8,
                        display: 'flex',
                        gap: 0.5,
                    } }, icons.map((iconDef, i) => (React.createElement(React.Fragment, { key: i }, WidgetUniversal.renderIcon(iconDef, iconStates[i], isWide ? 28 : 18)))))) : null,
                size === '1x1' ? (React.createElement(React.Fragment, null,
                    React.createElement(Box, { sx: {
                        ...
                        iconSrc ? (React.createElement(Icon, { src: iconSrc, style: {
                                width: iconSize,
                                height: iconSize,
                                color: valueColor || undefined,
                                flexShrink: 0,
                            } })) : null,
                        valueEl != null ? (React.createElement(Typography, { sx: {
                            ...
                            valueEl,
                            unit ? (React.createElement(Typography, { component: "span", sx: { fontSize: '0.45em', fontWeight: 400, ml: 0.5, opacity: 0.7 } }, unit)) : null)) : (this.state.commonType !== 'boolean' && (React.createElement(Typography, { sx: { fontSize: 32, color: 'text.disabled' } }, "\u2014")))),
                    this.props.settings.name ? (React.createElement(Typography, { sx: {
                            ...
                            whiteSpace: 'nowrap',
                            maxWidth: '90%',
                        } }, this.props.settings.name)) : null)) : (React.createElement(Box, { sx: {
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        gap: 2,
                    ...
                    React.createElement(Box, { sx: {
                        ...
                        valueEl != null ? (React.createElement(Typography, { sx: {
                            ...
                            valueEl,
                            unit ? (React.createElement(Typography, { component: "span", sx: { fontSize: '0.5em', fontWeight: 400, ml: 0.5, opacity: 0.7 } }, unit)) : null)) : (this.state.commonType !== 'boolean' && (React.createElement(Typography, { sx: { fontSize: isWideTall ? 36 : 24, color: 'text.disabled' } }, "\u2014")))))))));
    }
    renderCompact() {
    ...
    renderUniversalChartDialog() {
        ...
        const quickData = useQuick
            ? [{ data: this.state.quickSamples, color, name: title, unit: this.state.unit }]
            : undefined;
        const windowSec = Math.max(QUICK_CHART_MIN_WINDOW_SEC, Number(this.props.settings.quickChartInterval) || QUICK_CHART_DEFAULT_WINDOW_SEC);
        return (React.createElement(ChartDialog, { open: true, onClose: () => this.setState({ chartOpen: false }), title: title, historyIds: hasHistory ? [{ id: this.state.historyId, color, name: title }] : [], historyInstance: this.state.historyInstance, socket: this.props.stateContext.getSocket(), unit: this.state.unit, isFloatComma: this.props.stateContext.isFloatComma, widgetId: String(this.props.widget.id), instanceId: this.props.stateContext.instanceId, quickData: quickData, isBoolean: this.state.commonType === 'boolean', lockedWindowMs: useQuick ? windowSec * 1000 : undefined }));
    }
    render() {
        const isEditing = !!this.props.onOpenSettings;
