...
export class WidgetTemperature extends WidgetGeneric {
    ...
    renderTileStatus() {
        ...
        return (React.createElement(Box, { sx: { display: 'flex', alignItems: 'center', gap: 0.5 } },
            ...
            humidity != null ? (React.createElement(Typography, { variant: "caption", sx: {
                ...
                Math.round(humidity),
                "% ",
                WidgetTemperature.getHumidityEmoji(humidity))) : null));
    }
    renderTileAction() {
        const { humidity } = this.state;
        return (React.createElement(Box, { sx: { display: 'flex', alignItems: 'baseline', gap: 1.5 } },
            React.createElement(Typography, { variant: "h5", sx: { fontWeight: 700, whiteSpace: 'nowrap' } }, this.formatTemperature()),
